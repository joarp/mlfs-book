...
def backfill_predictions_for_monitoring(weather_fg, air_quality_fg, monitor_fg, model):
    weather_df = weather_fg.read()
    air_quality_df = air_quality_fg.read()

    # Select relevant columns
    weather_sel = weather_df[[
        "temperature_2m_mean",
        "precipitation_sum",
        "wind_speed_10m_max",
    ...
    features_df = features_df.apply(lambda g: g.tail(31).head(30))
    features_df['predicted_pm25'] = model.predict(features_df[['lag1', 'lag2', 'lag3', 'rolling','temperature_2m_mean', 'precipitation_sum', 'wind_speed_10m_max', 'wind_direction_10m_dominant']])
    df = pd.merge(features_df, air_quality_df[['date','city','pm25']], on=["city","date","pm25"], how="left")
...
def plot_air_quality_forecast_two_models(city: str, df: pd.DataFrame, file_path: str, mse: float, old_mse: float, r2: float, old_r2: float, hindcast=False):
    fig, ax = plt.subplots(figsize=(10, 6))

    day = pd.to_datetime(df['date']).dt.date
    # Plot each column separately in matplotlib
    ax.plot(day, df['predicted_pm25'], label=f'Predicted PM2.5 (MSE={mse}, R2={r2})', color='orange', linewidth=2, marker='o', markersize=5, markerfacecolor='blue')
    ...
    if hindcast == True:
        ax.plot(day, df['pm25'], label='Actual PM2.5', color='black', linewidth=2, marker='^', markersize=5, markerfacecolor='grey')
        ax.plot(day, df['predicted_old_pm25'], label=f'Old Predicted PM2.5 (MSE={old_mse}, R2={old_r2})', color='red', linewidth=2, marker='^', markersize=5, markerfacecolor='grey')
        legend2 = ax.legend(loc='upper left', fontsize='x-small')
        ax.add_artist(legend1)
